...
public class HttpListenerWorkerAttributeWriteHandler extends ReloadRequiredWriteAttributeHandler {
    ...
    protected void finishModelStage(OperationContext context, ModelNode operation, String attributeName, ModelNode newValue,
        ...
        context.addStep(new OperationStepHandler() {
            ...
            @Override
            public void execute(OperationContext stepContext, ModelNode op) throws OperationFailedException {
                final ModelNode remotingWorker = getRemotingWorker(stepContext);
                if (isHttpUpgradeEnabled(stepContext) && !isNewWorkerEqualToRemoting(remotingWorker, newValue)) {
                    stepContext.addResponseWarning(Level.WARNING, UndertowLogger.ROOT_LOGGER
                            .workerValueInHTTPListenerMustMatchRemoting(asString(newValue), asString(remotingWorker)));
                    stepContext.setRollbackOnly();
                }
            }
    ...
    private boolean isNewWorkerEqualToRemoting(final ModelNode remotingWorker, final ModelNode newValue) {
        if ((!newValue.isDefined() && !remotingWorker.isDefined()) || (newValue.isDefined() && remotingWorker.isDefined()
                && newValue.asString().equals(remotingWorker.asString()))) {
            return true;
        } else {
            return false;
        }
