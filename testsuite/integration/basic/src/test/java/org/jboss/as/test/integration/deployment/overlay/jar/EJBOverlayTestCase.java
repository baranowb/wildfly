...
@RunAsClient
public class EJBOverlayTestCase {
    @ArquillianResource
    protected ManagementClient managementClient;

    private static final Logger logger = Logger.getLogger(EJBOverlayTestCase.class);

    @Deployment(name = RemoteOverlayCheck.DEPLOYMENT_NAME_JAR, order = 0, managed = true, testable = false)
    ...
    public void setup() throws Exception {
        OverlayUtils.setupOverlay(managementClient, RemoteOverlayCheck.DEPLOYMENT_JAR_NAME_OVERLAYED, RemoteOverlayCheck.OVERLAY,
                RemoteOverlayCheck.OVERLAY_RESOURCE_JAR, RemoteOverlayCheck.VALUE_OVERLAYED);
    }

    @After
    public void cleanup() throws Exception {
        OverlayUtils.removeOverlay(managementClient, RemoteOverlayCheck.DEPLOYMENT_JAR_NAME_OVERLAYED, RemoteOverlayCheck.OVERLAY,
                RemoteOverlayCheck.OVERLAY_RESOURCE_JAR);
    }

    @Test
    public void testOverlay() throws Exception {
        RemoteOverlayCheck check = (RemoteOverlayCheck) getInitialContext().lookup(RemoteOverlayCheck.EJB_LOOKUP_NAME_JAR);
