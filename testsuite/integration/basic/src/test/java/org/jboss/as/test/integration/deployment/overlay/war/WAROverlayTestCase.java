...
package org.jboss.as.test.integration.deployment.overlay.war;
...
@RunAsClient
public class WAROverlayTestCase {
    @ArquillianResource
    protected ManagementClient managementClient;
    private static final Logger logger = Logger.getLogger(EJBOverlayTestCase.class);

    @Deployment(name = RemoteOverlayCheck.DEPLOYMENT_NAME_WAR, order = 0, managed = true, testable = false)
    ...
    public static Archive<?> createOverlayedArchive() {
        ...
        WebArchive war = ShrinkWrap.create(WebArchive.class, RemoteOverlayCheck.DEPLOYMENT_WAR_NAME_OVERLAYED);
    ...
    public void setup() throws Exception {
        OverlayUtils.setupOverlay(managementClient, RemoteOverlayCheck.DEPLOYMENT_WAR_NAME_OVERLAYED, RemoteOverlayCheck.OVERLAY,
                RemoteOverlayCheck.OVERLAY_RESOURCE_WAR, RemoteOverlayCheck.VALUE_OVERLAYED);
    }

    @After
    public void cleanup() throws Exception {
        OverlayUtils.removeOverlay(managementClient, RemoteOverlayCheck.DEPLOYMENT_WAR_NAME_OVERLAYED, RemoteOverlayCheck.OVERLAY,
                RemoteOverlayCheck.OVERLAY_RESOURCE_WAR);
    }

    @Test
    public void testOverlay() throws Exception {
        RemoteOverlayCheck check = (RemoteOverlayCheck) getInitialContext().lookup(RemoteOverlayCheck.EJB_LOOKUP_NAME_WAR);
